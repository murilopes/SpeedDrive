...
const  AgendamentosPorAlunoLista = () => {
  ...
  const _handleAlunoRealizadas = (idAluno: string) => {
    navigation.navigate('AlunoRealizadas', {idAlunoImpersonate: idAluno});
  };
  const _handleAlunoProximas = (idAluno: string) => {
    navigation.navigate('AlunoProximas', {idAlunoImpersonate: idAluno});
  };
  ...
  return (
    <KeyboardAvoidingView style={styles.container_principal} 
      ...
      <Appbar.Header statusBarHeight={0} style={{height: 60, backgroundColor: '#212F3C'}}>
        ...
        <Appbar.Content  title="Agendamentos alunos" style={{alignItems:'center'}}/>
            ...
            arrayAlunos.map((item, i) => (
              <View key={item._id} style={styles.item}>
                ...
                <View style={styles.item_action}>
                  <RectButton style={styles.button} onPress={() => _handleAlunoRealizadas(item._id)}>
                    <Text style={styles.buttonText}>Histórico</Text>
                  </RectButton>
                  <RectButton style={styles.button} onPress={() => _handleAlunoProximas(item._id)}>
...
export default AgendamentosPorAlunoLista;
